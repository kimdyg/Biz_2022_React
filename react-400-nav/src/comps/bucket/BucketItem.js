...
function BucketItem(props) {
  ...
  데이터를 표출하기 위하여 변수={값} 형식으로 자식 comp에게 전달하고
  자식comp는 props를 통해 데이터를 받아서 표출한다
  이때 만약 자식 comp 에서 부모 comp 가 생성 선언한 state 변수를 변경해야 할 경우가 발생하면
  부모 comp 에서 state 변수를 변경하는 함수를 함께 만들어
  자식 comp 에게 전달해 주어야한다

  지금 <td onClick = {()=>bucket_complete(id)}> 의 코드를 보면
  현재 화면(comp)에 선언된 td가 클릭 되었을때 bucket_complete 함수를 
  실행하고 있지만 이 함수는 BuckectMain 에서 만들어 내려보낸 함수이다
   */
...
/* 
event 핸들러에게 어떤 값을 전달하면서 실행하고자 할때 다음과 같은 코드는 절대
사용하면 안된다
<td onClick={bucket_complete(item.b_id)}></td>
위의 코드는 event 와 관계없이 화면이 redering 될때 내부의 함수가 실행되어 버린다

event 핸들러에 어떤 값을 전달하면서 실행하기
위해서는 한번 더 함수를 감싸줘야 한다
<td onClick={()=>bucket_complete(item.b_id)}></td>
