/* 
use* 로 시작되는 react의 함수들 
Hook 함수라고 한다
component lifecycle 과 관련된 함수들이다
state 만들거나, state 변화를 감시하거나
component 가 rendering 되는 시점에 무언가를 실행시키거나
하는 용도의 함수들 이다
*/
...
이때 return 내의 컴포넌트 tag 의 가장 바깥쪽을 감싸서
한개의 tag로 묶어야 한다
*/
const BucketMain = () => {
  /* 
  ...
  const bucket_flag_toggle = (id) => {
    ...
    const bucketBody = bucketList.map((bucket) => {
      if (bucket.b_id === id) {
        ...
        // b_flag 값만 원의 b_flag 값에 1 을 덧셈하여 변경하고
        // 새로운 _temp_bucket 에 복제해라
      ...
      // id 가 일치하지 않는 데이터는 변경하지말고 원래 데이터를 return
      return bucket;
    }); // end map
