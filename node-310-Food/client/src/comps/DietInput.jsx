...
const DietInput = () => {
  ...
  */
  ...
  const onClick = async (e) => {
    ...
    CORS(Cross Origin Resource Share, 교차사이트 스크립트 오류)가 발생을한다
    서버와 서버간의 통신(데이터교환)이 이루어질때
    보안, 해킹 방지 등을 이유로 점점 많은 곳에서 스크립트 교환이 이루어지지 않도록 하고있다
    CORS 때문에 실제 필요한 API 이용에 제한 많다
    React, NodeJS 가 같은 서버에서 작동 될때는 Package.json 에 
    proxy 설정을 통하여 CORS 문제를 일부 해결 할 수 있다

    proxy 설정을 했을 경우는 fetch URL 부분에 http://localhost:3000 주소를 생략하고
    router 주소만 사용하여 nodejs 와 데이터를 주고 받는다.
    */
